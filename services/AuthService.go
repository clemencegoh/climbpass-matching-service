...
type IAuthService interface {
	...
	DeleteAuthByID(id int) ([]byte, error)
	UpdateAuthByID(id int, auth models.AuthUser) ([]byte, error)
}
...
func (service AuthService) AuthenticateUser(auth models.AuthUser) ([]byte, error) {
	existing := service.repository.GetAuthByUsername(auth.Username)
	if existing.ID == 0 || existing.Password != auth.Password {
		// User not found
...
func (service AuthService) DeleteAuthByID(id int) ([]byte, error) {
	service.repository.DeleteAuthByID(id)
	return []byte(fmt.Sprintf("%v", id)), nil
}

// UpdateAuthByID updates with new object
func (service AuthService) UpdateAuthByID(id int, user models.AuthUser) ([]byte, error) {
	user.ID = id
